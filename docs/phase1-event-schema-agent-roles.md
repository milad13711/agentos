...
## قدم ۵ — دیپلوی روی Production ✅ انجام شد

روی سرور `94.182.93.52` اجرا و تأیید شد:
- بکاپ گرفته شد (`~/agentos-backup-*.sqlite`)
- کد جدید build و deploy شد؛ در همین مسیر دو باگ جدا (بی‌ربط به خودِ migration) پیدا و فیکس شد:
  - `agentos-web` healthcheck fail می‌کرد چون Next.js standalone روی HOSTNAME خودکار Docker (container ID) bind می‌شد نه `0.0.0.0`
  - AI Gateway (`callAnthropic`/`callOpenAI`) هیچ timeout ای نداشت — وقتی GapGPT کند/بی‌پاسخ می‌شد، Agent برای همیشه "در حال پردازش" می‌موند بدون هیچ خطایی در لاگ
  - Dockerfile بک‌اند پوشه‌ی `scripts/` رو کپی نمی‌کرد، پس اسکریپت backfill داخل کانتینر نبود
- backfill بدون حذف اجرا شد: `audit_logs rows: 8, newly inserted into events: 8` — محتوا دستی تأیید شد (ثبت‌نام‌ها، ساخت مخاطب/ماژول، تایید، تغییر پرسونا)
- بعد از تأیید، `--drop-legacy-tables` اجرا شد — `audit_logs`/`pending_actions` حذف شدن
- تأیید نهایی: لیست جدول‌ها فقط `events` رو نشون می‌ده (نه جدول‌های قدیمی)، و `/api/health` سالمه

### (بخش قدیمی زیر فقط برای مرجع، دیگر لازم نیست دوباره اجرا شود)
