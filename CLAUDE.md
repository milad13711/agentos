...
8. **درخواست بزرگ کاربر (۲۰۲۶-۰۷-۲۵) — فقط فاز ۱ (باگ‌های فوری + اتوماسیون ماژول) انجام شد، ۷ آیتم دیگه هنوز باقی‌مونده:**
   ...
   - ⏳ **باقی‌مونده (کاربر صراحتاً برای فاز بعد گذاشت، تصمیم‌ها از قبل گرفته شده):**
     ...
     - یادآوری خودکار وظایف در ساعت/تاریخ مشخص — کاربر کانال **تلگرام** رو برای فاز اول انتخاب کرد (نه پوش/پیامک). نیاز به یک worker/cron که `tasks.due_at` رو چک کنه و وقتی سررسید شد پیام بفرسته — می‌تونه از همون Bot توکن آیتم بعدی استفاده کنه.
     - اتصال Agent به تلگرام — کاربر **بات رسمی (BotFather)** رو انتخاب کرد، نه اکانت شخصی با شماره تلفن (که ریسک بن‌شدن و مشکل ToS داشت). یعنی: ساخت بات با BotFather، وبهوک/polling، مپ‌کردن chat_id تلگرام به یک `users.id` (احتمالاً یک کد تایید یک‌بارمصرف موقع اتصال اول)، و بعد پیام‌های تلگرام کاربر رو از همون مسیر `agent.js`'s `act()` رد کردن.
